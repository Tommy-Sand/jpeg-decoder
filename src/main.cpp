    ...
    */
	...
	for(int i = 0; i < 2; i++){
		for(int j = 0; j < 2; j++){
			...
			std::cout << "length: " << (int) huffman_table.get_length() << std::endl;
			std::cout << "type: " << (int) huffman_table.get_type() << std::endl;	
			std::cout << "table_id: " << (int) huffman_table.get_table_id() << std::endl;
			for(int i = 0; i < 16; i++){
				std::cout << "Num of codes for " << i << ": "  << (int) huffman_table.get_num_codes_len_i()[i] << "\t Min code value: " << (int) huffman_table.get_min_code_value()[i] << "\t Max code value: " << (int) huffman_table.get_max_code_value()[i] << std::endl;
