...
HuffTables *new_huff_tables(Encoding process) {
	...
	if (process == BDCT) {
		...
		hts->nDCAC = 2;
		if ((hts->DCAC[0] = (HuffTable *) malloc(sizeof(HuffTable) * 2)) == NULL) {
			free(hts);
			return NULL;
		}
		if ((hts->DCAC[1] = (HuffTable *) malloc(sizeof(HuffTable) * 2)) == NULL) {
			free(hts->DCAC[0]);
			free(hts);
			return NULL;
		}
		...
		for (uint8_t i = 0; i < 2; i++) { 
			...
			for (uint8_t j = 0; j < hts->nDCAC; j++) {
				...
				for (uint8_t k = 0; k < 16; k++) {
					printf("len: %d\n", ht->len[k]);
					if (ht->len[k] == 0) {
						printf("\t\tk: %d, min_codes: %X, max_codes: %X, len: %d\n", k + 1, ht->min_codes[k], ht->max_codes[k], ht->len[k]);
						continue;
					}
					printf("\t\tk: %d, min_codes: %X, max_codes: %X, len: %d\n", k + 1, ht->min_codes[k], ht->max_codes[k], ht->len[k]);
					printf("\t\tsymbols: ");
